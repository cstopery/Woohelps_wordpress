...
<div id="post-<?php bbp_reply_id(); ?>" class="bbp-reply-header">
    <?php if (is_user_logged_in() && bbp_current_user_can_access_create_topic_form() && (strpos($class,
                'bbp-reply-position-1') || strpos($class, 'type-topic'))
    ): ?>
...
$meta['enroll_way'] = get_post_meta($topic_id, 'enroll_way', true);
?>

<div <?php bbp_reply_class(); ?>>

    <div class="<?= (strpos($class, 'bbp-reply-position-1') || strpos($class,
            'type-topic')) ? 'bbp-reply-content' : ''; ?>">

        <h4 class="bbp-reply-title">
            <?= (strpos($class, 'bbp-reply-position-1') || strpos($class,
                    'type-topic')) ? '<a href="' . bbp_get_topic_permalink() . '">' . bbp_get_topic_title() . '</a>' : ''; ?>
        </h4>

        <div class="bbp-reply-inner-content">
            <h5>
                <?= (!strpos($class, 'bbp-reply-position-1') && !strpos($class,
                        'type-topic')) ? bbp_get_reply_author_link() : ''; ?>
            </h5>
        </div>
                        ...
                        <?php foreach ($subscribers as $id): $user = get_user_by('id', $id); ?>
                            <li>
                                ...
                                <span class="attendee-info">
                                    <?= bbp_get_user_nicename($user->ID) ?> 预定 <?= get_user_meta($user->ID,
                                        'subscribe-' . $topic_id, true) ?> 个名额
                                </span>
                            </li>
                        <?php endforeach; ?>
                    </ul>
        ...
        <div class="table-responsive">
            <table class="table table-hover">
                <tr>
                    ...
                    <td><?= date('Y 年 m 月 d 日',
                            $meta['date_and_time'] / 1000 + get_option('gmt_offset') * HOUR_IN_SECONDS); ?></td>
                </tr>
                ...
                <tr>
                    ...
                    <td><?= $meta['attendee_count_limit'] ?>
                        （剩余 <?= $meta['attendee_count_limit'] - $meta['attendee_count'] ?>）
                    </td>
                </tr>
            ...
            <?php if (is_user_logged_in()) { ?>
                ...
                if (!$is_subscribed && ($meta['attendee_count_limit'] - $meta['attendee_count'] > 0)) {
                    ...
                    <div id="resv">
                        ...
                        <input type='text' name='resv' id="resv-<?= $topic_id ?>" value='1' class='attendee_count'
                               disabled/>
                        <button class='btn btn-default plus' id="plus-<?= $topic_id ?>">+</button>
                        <input type="hidden" id="left-<?= $topic_id ?>"
                               value="<?= $meta['attendee_count_limit'] - $meta['attendee_count'] ?>">
                    </div>
                <?php } ?>
                <?php  if ($meta['enroll_way'] == "1"): ?>
                    <div class="subscription-button">
                        <?php bbp_topic_subscription_link(); ?>
                    </div>
                <?php elseif($meta['enroll_way'] == "2"): ?>
                    <div>
                       请在活动描述中查看报名方式!
                    </div>
                <?php else : ?>
